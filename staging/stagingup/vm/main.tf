...
data "terraform_remote_state" "network" { // This is to use Outputs from Remote State
  backend = "s3"
  config = {
    bucket = "staging-group13"                   // Bucket from where to GET Terraform State
...
resource "aws_instance" "my_amazon" {
  ...
  root_block_device {
    encrypted = var.env == "staging" ? true : false
  }
...
resource "aws_key_pair" "key1" {
  key_name   = "key1"
  public_key = file("/home/ec2-user/environment/staging/vm/key1.pub")
}
...
resource "aws_key_pair" "key2" {
  ...
  public_key = file("/home/ec2-user/environment/staging/vm/key2.pub")
}
...
resource "aws_autoscaling_group" "staging" {
  name = "${aws_launch_configuration.staging.name}-asg"
  min_size             = 1
  desired_capacity     = 3
  max_size             = 4
...
#auto scaling policy
resource "aws_autoscaling_policy" "staging_policy_up" {
  name = "staging_policy_up"
  scaling_adjustment = 1
  adjustment_type = "ChangeInCapacity"
  cooldown = 300
  autoscaling_group_name = "${aws_autoscaling_group.staging.name}"
}
resource "aws_cloudwatch_metric_alarm" "staging_cpu_alarm_up" {
  alarm_name = "staging_cpu_alarm_up"
...
alarm_description = "This metric monitor EC2 instance CPU utilization"
  alarm_actions = [ "${aws_autoscaling_policy.staging_policy_up.arn}" ]
}
resource "aws_autoscaling_policy" "staging_policy_down" {
  name = "staging_policy_down"
  scaling_adjustment = -1
  adjustment_type = "ChangeInCapacity"
  cooldown = 300
  autoscaling_group_name = "${aws_autoscaling_group.staging.name}"
}
resource "aws_cloudwatch_metric_alarm" "staging_cpu_alarm_down" {
  alarm_name = "staging_cpu_alarm_down"
...
alarm_description = "This metric monitor EC2 instance CPU utilization"
  alarm_actions = [ "${aws_autoscaling_policy.staging_policy_down.arn}" ]
}
